...
##### 높이(px)와 스크롤/폭(px) 옵션 제공
st.title("대전시 지표면 3D 시각화 (HTML 임베드)")
height = st.sidebar.number_input("표시 높이(px)", min_value=400, max_value=3000, value=900, step=50)
scrolling = st.sidebar.checkbox("스크롤 허용", value=True)
width = st.sidebar.number_input("프레임 폭(px)", min_value=600, max_value=3840, value=1600, step=50)

if not HTML_PATH.exists():
    st.error(f"HTML 파일을 찾지 못했습니다: {HTML_PATH.resolve()}")
else:
    html_str = HTML_PATH.read_text(encoding="utf-8", errors="ignore")
    st.components.v1.html(html_str, height=height, scrolling=scrolling, width=width)
